...
function mixinEnvConfig(env, requirejsConfig) {
  ...
  // These are only used when the cache is off-line
  var fallbackEndpoints = [ "navigation", "business" ];
  fallbackEndpoints.forEach(function(name) {
    moduleConfig["components/content/entities/specializations/"+name+"/entities"] = {
      endpoint: envConfig.atf[name]
    };
  });
